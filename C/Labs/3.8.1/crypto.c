...
{
	FILE *fichierSource = fopen(source, "r");
	
	if (fichierSource != NULL)
	{
		FILE *fichierDestination = fopen(destination, "w");
		
		if (fichierDestination != NULL)
		{
			char buffer[1024];
			char *phraseChiffree = NULL;
			
			while (fgets(buffer, 1023, fichierSource) != NULL)
			{
				phraseChiffree = chiffrer_phrase(a, b, buffer);
				fputs(phraseChiffree, fichierDestination);
				free(phraseChiffree);
			}
		...
		{
			fclose(fichierSource);
			printf("<ERROR> Le fichier '%s' n'existe pas ou ne peut pas etre cree.\n", destination);
		}
	}
	else
	{
...
void dechiffrer_fichier(int a, int b, const char *source, const char *destination)
{
	...
	if (srcFile == NULL)
	{
		printf("\nImpossible d'ouvrir le fichier \"%s\", ce fichier n'existe pas.", source);
	}
	else
	{
		...
		if (dstFile == NULL)
		{
			printf("\nImpossible de créer ou d'ouvrir le fichier \"%s\".", destination);
			fclose(srcFile);
		}
		else
		{
			// Déchiffrer les lignes une par une.
			char cryptedLine[1024]; // On dimensionne notre buffer de manière raisonable.
			char* clearLine;
			while (fgets(cryptedLine, 1023, srcFile) != NULL)
			{
				clearLine = dechiffrer_phrase(a, b, cryptedLine);
				if (fputs(clearLine, dstFile) != EOF)
				{
					free(clearLine);
				}
				else
				{
					free(clearLine);
					printf("\nImpossible d'écrire dans le fichier \"%s\". Opération abandonnée.", destination);
					break;
				}
			}
